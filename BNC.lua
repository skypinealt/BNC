...
local function checkMissingDependencies(testName, dependencies)
    local missing = {}
    for _, dep in ipairs(dependencies) do
        if not getGlobalValue(dep) then
            table.insert(missing, dep)
        end
    end

    if #missing > 0 then
        warn("⚠️ " .. testName .. " missing dependencies: " .. table.concat(missing, ", "))
    end
end

local function runTest(testName, aliases, testCallback, dependencies)
    testsRunning += 1
    ...
    task.spawn(function()
        if not testCallback then
            ...
            local globalValue = getGlobalValue(testName)
            if not globalValue then
                ...
                if success then
                    ...
                    totalFails += 1
                    checkMissingDependencies(testName, dependencies or {})
...
local function printSummary()
    print("\n")
    print("BNC Environment Check - " .. tostring(identifyexecutor()))
    print("✅ - Pass, ❌ - Fail, 🚫 - No test, ⚠️ - Missing aliases\n")

    task.defer(function()
        repeat
            task.wait()
        until testsRunning == 0
        ...
        print("⚠️ " .. tostring(totalUndefined) .. " missing aliases")
    end)
end
...
runTest("decompile", {"saveinstance"}, function()
...
runTest("request", {"http.request", "http_request"}, function()
    ...
    assert(response.StatusCode == 400, "Did not return a 400 status code")
...
runTest("setscriptbytecode", {}, function()
    local script = Instance.new("LocalScript")
